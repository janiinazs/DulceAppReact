...
import OwnerNavigator from './src/navigation/OwnerNavigator'; // Panel del Dueño
...
const RootNavigator = () => {
  ...
  const { userToken, userRole, isLoading, lastUserRole } = useContext(AuthContext);
  ...
  return (
    ...
    <NavigationContainer>
      ...
      {userToken ? (
        ...
        userRole === 'owner' ? <OwnerNavigator /> : <DashboardTabs />

      ) : (

        <AuthNavigator initialRouteName={lastUserRole === 'owner' ? 'OwnerLogin' : 'Login'}/>
